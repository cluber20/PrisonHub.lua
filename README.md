...
local Settings = {
    AimbotFOV      = 150,  -- larger default for mobile
    AimbotSmoothing = 0.3, -- how fast cam snaps (0=instant, 1=slow)
    AimbotPart     = "Head",
...
local function StartAimbot()
    ...
    AimbotConnection = RunService.RenderStepped:Connect(function()
        ...
        if AimbotTarget and AimbotTarget.Character then
            ...
            if part then
                if Toggles.AimbotSilent then
                    -- Silent Aim: don't move camera, intercept shots instead
                    -- (handled in namecall hook below)
                else
                    -- Camera Aimbot: smoothly snap to target
...
-- FOV Circle (mobile friendly, drawn via Drawing API)
...
namecall = hookmetamethod(game, "__namecall", function(self, ...)
    ...
    -- Silent Aim: redirect ray/target to AimbotTarget
...
local function JoinLowestPingServer()
    ...
    local function getIgnored()
        ...
        local ignored, ok, content = {}
        ok, content = pcall(readfile, IGNORE_FILE)
...
AimbotTab:CreateSlider({
    ...
    Flag         = "AimbotSmooth",
    Callback     = function(val)
        -- map 1-10 → 0.05-0.95 (inverted: higher = faster for mobile feel)
        Settings.AimbotSmoothing = val / 10
    end,
})
